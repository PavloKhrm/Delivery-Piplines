...
export class BlogPostService {
  ...
  async ensureIndex() {
    // Skip Elasticsearch operations if not available
    console.log('ELASTICSEARCH_HOST:', process.env.ELASTICSEARCH_HOST);
    if (!process.env.ELASTICSEARCH_HOST || process.env.ELASTICSEARCH_HOST.includes('localhost')) {
  ...
  async findAll(
    ...
    // Always use database search for now - Elasticsearch indexing needs to be set up
    if (true) {
