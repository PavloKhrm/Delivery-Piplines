...
// ADD THIS NEW EVENT LISTENER FOR THE CLIENT CREATION FORM
document.getElementById("create-client-form").addEventListener("submit", async (e) => {
    e.preventDefault();
    const clientName = document.getElementById("clientName").value;
    const output = document.getElementById("client-create-output");
    output.textContent = `Starting creation for client: ${clientName}...`;

    try {
        const res = await fetch("/api/create-client", {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Basic ${btoa(localStorage.auth)}` },
        ...
        if (!res.ok) {
            ...
            document.getElementById("clientName").value = ""; // Clear input on success
        }
    } catch (err) {
        output.textContent = `A network error occurred: ${err.message}`;
    }
});
