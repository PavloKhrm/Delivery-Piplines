...
const fetchPost = async () => {
  loading.value = true;
  try {
    const res = await fetch(`/api/blog-post/${route.params.id}`, { credentials: 'include' });
    if (res.ok) {
...
const updatePost = async () => {
    const res = await fetch(`/api/blog-post/${route.params.id}`, {
    method: 'PATCH',
    headers: {
...
const deletePost = async () => {
    await fetch(`/api/blog-post/${route.params.id}`, { method: 'DELETE', credentials: 'include' });
