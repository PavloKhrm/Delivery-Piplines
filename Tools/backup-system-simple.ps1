# Multi-Tenant Kubernetes Blog Template System - Simple Backup Script
...
function Export-KubernetesManifests {
    Write-Host "Exporting Kubernetes manifests..." -ForegroundColor Yellow
...
function Export-DatabaseDumps {
    Write-Host "Exporting database dumps..." -ForegroundColor Yellow
    ...
    foreach ($namespace in $blogNamespaces) {
        ...
        try {
            ...
            if ($mysqlPod) {
                ...
                Write-Host "    No MySQL pod found for $nsName" -ForegroundColor Yellow
            }
        } catch {
...
function Create-BackupManifest {
    ...
    $manifest = @{
        ...
        system = @{
            ...
            kubectl = (kubectl version --client).Trim()
            helm = (helm version).Trim()
        }
        namespaces = @()
...
try {
    ...
    Write-Host "`nTo restore this backup, run:" -ForegroundColor Cyan
    Write-Host ".\restore-system.ps1 -BackupName `"$BackupName`"" -ForegroundColor White
    
} catch {
    Write-Host "`nBackup failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
